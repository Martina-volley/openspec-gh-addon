---
name: openspec-gh-new
description: Start a new OpenSpec change with GitHub integration. Includes preflight checks, base branch confirmation, branch creation, and artifact requirement guidance by branch type.
license: MIT
compatibility: Requires openspec CLI and git CLI.
metadata:
  author: custom
  version: "1.2"
  basedOn: openspec-new-change
---

Start a new change with automatic Git branch creation and GitHub push.
...
- **`explore/` branches**: Ask:
  > "要不要 push 到 GitHub？（explore 分支通常用於本地快速驗證，可以之後再 push）"
...
📋 依 <branch-type>/ 分支類型，最低 artifact 要求：
  - <artifact 1>
  - <artifact 2>
建議完成這些 artifacts 後再執行 /opsx-gh-apply。
```
